...
const CAMP = JSON.parse(fs.readFileSync(path.join(ROOT, 'app', 'data', 'campsites.json'), 'utf8'));
{
  ...
  // line and its two-space closing brace, so re-indenting it makes the suite throw, not fail.
  {
    ...
    ok('a link is labelled by the agency that published it, never by another agency',
       ...
       `an unrecognised host reads "${unknown.text}"`);

    // Card 0076 #3. This sweep read DATA.sites only, so the label logic was proved over one
    // data file and asserted over the other by a single fixture -- the `stn` case above.
    // The Stay the Night car parks are 44 real records carrying forestryandland.gov.scot
    // URLs and they live in app/data/campsites.json, which is a separate file under a
    // separate licence and is regenerated by a separate pipeline. Selected by HOST rather
    // than by country, because a Scottish campsite from OpenStreetMap carries its own
    // business website and is correctly labelled by that host, not as an agency.
    //
    // The host test is deliberately LOOSE: any mention of the host, in any case, under any
    // scheme or none. The first version required exactly `https://host/`, so an `http://`
    // or bare-host record was not selected, not counted and not checked, while the
    // non-zero guard still passed on the others. Over-selecting only makes this stricter:
    // a record it wrongly picks up must still read as Forestry and Land Scotland, and an
    // `http://` one, which safeHref refuses to link at all, fails here by name.
    const mentionsFls = s => /forestryandland\.gov\.scot/i.test(s.url || '');
    const flsSites = sites.filter(s => (s.country === 'Scotland' && s.url) || mentionsFls(s));
    const flsCamp = CAMP.sites.filter(mentionsFls);
    const mislabelled = flsSites.concat(flsCamp)
      .filter(s => link(s, s.url).text !== 'Forestry and Land Scotland page');
    // A non-zero count is required from EACH file, so neither half can pass by reading
    // nothing. A sweep that is green because its input was empty is this project's
    // recurring defect and the reason this card exists at all.
    // Named exactly as card 0076 #3 cites it, so the run log and the card agree.
    ok('every forestry and land scotland url is labelled by host, in both data files',
       flsSites.length > 0 && flsCamp.length > 0 && mislabelled.length === 0,
       `${mislabelled.length} records are not labelled Forestry and Land Scotland across ` +
       `${flsSites.length} in sites.json and ${flsCamp.length} in campsites.json` +
...
console.log('\n--- dataset counts carried in prose (card 0036) ---');
{
  ...
  // Card 0076. The PRD's per-site detail bullet promised "link to the Forestry England
  // page" for every site long after card 0057 made the label read off the link's own host,
  // and 276 of the 550 forests are Forestry and Land Scotland. A requirements document that
  // describes a removed defect invites somebody to restore it, and this is the document a
  // session reads before deciding whether a change is in scope.
  //
  // FLATTEN FIRST, and that is the whole point of this check rather than a detail of it.
  // The bullet wraps: "link to the" ends line 56 and "Forestry England page." begins line
  // 57, so `grep -c "link to the Forestry England page" docs/PRD.md` prints 0. That is not
  // a hypothetical -- it is how the finding was dismissed as not reproducing under card
  // 0069 after card 0057's reviewer had raised it correctly. A line-at-a-time search over
  // this document cannot see the sentence it is searching for.
  const prdFlat = fs.readFileSync(path.join(ROOT, 'docs', 'PRD.md'), 'utf8')
    .replace(/\s+/g, ' ');
  // Only the agencies this app actually scrapes or plans to. naturalresources.wales is on
  // the list on purpose: card 0017 would add a third agency, and the same sentence naming
  // that one alone would be the same defect.
  // Each agency is matched by its name OR its host, case-insensitively, because a sentence
  // naming forestryengland.uk promises the same wrong thing as one naming Forestry England.
  const PRD_AGENCIES = [/forestry england|forestryengland\.uk/i,
                        /forestry and land scotland|forestryandland\.gov\.scot/i,
                        /natural resources wales|naturalresources\.wales/i];
  // "link to", "links to", "linked to", "linking to", in any case. The first version
  // matched the exact lower-case words only, so "links to the Forestry England page" and
  // "Link to the Forestry England page" named one agency and passed (2026-09-17 review).
  const PRD_LINK = /\blink(?:s|ed|ing)? to\b/i;
  // Sentences end at a full stop FOLLOWED BY WHITESPACE or the end, never at every full
  // stop: splitting on each "." cut "forestryengland.uk" in half before the agency could be
  // read, which is how a sentence naming the agency by its host went unseen.
  const prdSentences = flat => flat.split(/(?<=\.)(?:\s+|$)/);
  // Every sentence that says what a site's link points at and names exactly one agency.
  // Sentence-wise rather than document-wise, because the PRD names Forestry England
  // legitimately all over -- in the summary, the licensing section and the trade-mark note
  // -- and a document-wide search would either fire on all of those or be narrowed until it
  // fired on nothing.
  const oneAgencySentences = flat => prdSentences(flat)
    .filter(s => PRD_LINK.test(s) && /per-site|site|forest/i.test(s))
    .filter(s => PRD_AGENCIES.filter(a => a.test(s)).length === 1);
  const oneAgency = oneAgencySentences(prdFlat);
  // A sentence-shaped check goes vacuous the moment the sentence is reworded out of reach,
  // so pin that the bullet it is written for is still there and still says where the link
  // goes. Without this, deleting the bullet is a green run.
  const prdBullet = prdSentences(prdFlat).find(s => /- Per-site detail:/i.test(s));
  const prdProblems = [];
  if (!prdBullet) prdProblems.push('docs/PRD.md carries no "Per-site detail:" bullet');
  else if (!PRD_LINK.test(prdBullet)) {
    prdProblems.push('the per-site detail bullet no longer says where the link goes: '
                     + JSON.stringify(prdBullet));
  }
  oneAgency.forEach(s => prdProblems.push(
    'names one agency for every link: ' + JSON.stringify(s.trim())));
  // Rewordings the 2026-09-17 review showed passing: each names one agency for every link
  // and each must be caught. Run through the same matcher as the real document, so the
  // matcher cannot be loosened without this going red.
  const prdOneAgencyMisses = [
    'links to the Forestry England page',
    'Link to the Forestry England page',
    'links to the\n  forestryengland.uk page',
  ].map(t => '- Per-site detail: name, sat-nav postcode, facilities, ' + t + '.')
    .filter(t => oneAgencySentences(t.replace(/\s+/g, ' ')).length === 0);
  prdOneAgencyMisses.forEach(t => prdProblems.push(
    'the check lets through one agency for every link: ' + JSON.stringify(t)));
  // Named exactly as card 0076 cites it, so the run log and the card agree.
  ok('the requirements document does not promise one agency for every forest link',
     prdProblems.length === 0, prdProblems.join(' | '));
