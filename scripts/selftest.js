...
const CAMP = JSON.parse(fs.readFileSync(path.join(ROOT, 'app', 'data', 'campsites.json'), 'utf8'));
{
  ...
  // line and its two-space closing brace, so re-indenting it makes the suite throw, not fail.
  {
    ...
    const flsCamp = CAMP.sites.filter(
      s => /^https:\/\/(www\.)?forestryandland\.gov\.scot\//.test(s.url || ''));
    const mislabelled = scots.concat(flsCamp)
    ...
    ok('a link is labelled by the agency that published it, never by another agency',
       ...
       scots.length > 0 && flsCamp.length > 0 && mislabelled.length === 0,
       `a Scottish forest reads "${fls.text}", an English one reads "${fe.text}", ` +
       `the same English host shouted reads "${shouty.text}", ` +
       `an unrecognised host reads "${unknown.text}", and ${mislabelled.length} records are ` +
       `labelled as another agency across ${scots.length} shipped Scottish records in ` +
       `sites.json and ${flsCamp.length} Forestry and Land Scotland records in campsites.json` +
...
console.log('\n--- dataset counts carried in prose (card 0036) ---');
{
  ...
  const PRD_AGENCIES = ['Forestry England', 'Forestry and Land Scotland',
                        'Natural Resources Wales'];
  // Every sentence in the document that says what a site's link points at. Sentence-wise
  // rather than document-wise, because the PRD names Forestry England legitimately all over
  // -- in the summary, the licensing section and the trade-mark note -- and a document-wide
  // search would either fire on all of those or be narrowed until it fired on nothing.
  const prdLinkSentences = (prdFlat.match(/[^.]*\blink to\b[^.]*\./g) || [])
    .filter(s => /per-site|site|forest/i.test(s));
  const oneAgency = prdLinkSentences
    .filter(s => PRD_AGENCIES.filter(a => s.includes(a)).length === 1);
  ...
  const prdBullet = (/- Per-site detail:[^.]*\./.exec(prdFlat) || [])[0];
  const prdProblems = [];
  if (!prdBullet) prdProblems.push('docs/PRD.md carries no "Per-site detail:" bullet');
  else if (!/\blink to\b/.test(prdBullet)) {
