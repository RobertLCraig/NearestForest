...
**Stage:** active
**Status:** Built, deployed and verified from a desk. 34 self-tests green, 904 sites. Everything
left either needs a physical iPhone (board `human-review/`) or is the car-park naming job, card
**0004**, which is agent-ready and the head of `todo/`.
_Last updated: 2026-08-08 (deployed to forestlocator.enhanceify.co.uk via the git-pull pattern)_
...
docroot symlinked into the checkout; the vhost PHP version pinned rather than inherited.
...
1. **0004** derive names for the 170 unnamed car parks (`todo/`) — **start here.** Agent-ready, no
   ...
   Brighton is currently one of the unnamed ones, which is the whole problem in a single row.
2. **0005** the deploy (`ai-review/`) — awaiting an adversarial pass. Read the deploy scripts
   sceptically before this moves to `done/`.
3. **0001 / 0002** (`human-review/`) — both now unblocked and waiting on Rob and a phone.
