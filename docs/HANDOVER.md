...
- **Both are generated, never hand-edited.** 1,180 records / 719 KB and 3,647 records / 964 KB,
...
- **A campsite never shows an open/closed badge.** 98 of 3,647 records publish any hours at all, so
...
Forestry and Land Scotland --------+                                                       |
  (1 index request + 278 pages)    |                                                       |
                          ...
                          scripts/parse.py                                 scripts/parse_campsites.py
                                   |                                                       |
                                   v                                                       v
                   app/data/sites.json (1,180)                       app/data/campsites.json (3,647)
                                   |                                                       |
                                   +---------------------+---------------------------------+
...
- **The campsite filter is where the judgement lives**, in `takes_a_van()` and the drop rules around
  it. 8,501 features in, 3,647 out, every exclusion counted and printed. Widening it to include the
...
- **Built and not yet deployed, as one batch:** **0004** (177 car parks named after their nearest
  ...
  **0022** (the footer credits), both 2026-09-05. 248 self-tests pass (`node scripts/selftest.js`,
  measured 2026-09-08) and `CACHE` / `BUILD` are at `v22-2026-09-08`, bumped five times by card 0020:
  ...
  scout sites that carry no `scout` tag, and when it dropped 26 residential parks and park-home
  estates that a static-brand list cannot see. See
...
1. **Look at 0004 and 0019 on a screen, then deploy 0004, 0015, 0016, 0019 and 0022 together.**
   **0019 and 0022 are the same paragraph**, four sentences of footer text, so the only thing to look
   ...
   Then `pwsh ./scripts/deploy.ps1`; `CACHE` and `BUILD` are already bumped to `v22-2026-09-08`.
...
4. **An adversarial pass over `ai-review/`** — twenty-nine cards on 2026-09-07 (`ls docs/board/ai-review`,
   ...
   whether 3,647 more markers have broken the map's clustering or its label collisions. Nothing reaches `done/` without somebody trying
