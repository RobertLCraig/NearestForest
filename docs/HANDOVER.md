...
- **Both are generated, never hand-edited.** 1,180 records / 719 KB and 3,675 records / 970 KB,
...
- **A campsite never shows an open/closed badge.** 98 of 3,675 records publish any hours at all, so
...
Forestry and Land Scotland --------+                                                       |
  (1 index request + 278 pages)    |                                                       |
                          ...
                          scripts/parse.py                                 scripts/parse_campsites.py
                                   |                                                       |
                                   v                                                       v
                   app/data/sites.json (1,180)                       app/data/campsites.json (3,675)
                                   |                                                       |
                                   +---------------------+---------------------------------+
...
- **The campsite filter is where the judgement lives**, in `takes_a_van()` and the drop rules around
  it. 8,501 features in, 3,675 out, every exclusion counted and printed. Widening it to include the
...
- **Built and not yet deployed, as one batch:** **0004** (177 car parks named after their nearest
  ...
  **0022** (the footer credits), both 2026-09-05. 241 self-tests pass (`node scripts/selftest.js`,
...
4. **An adversarial pass over `ai-review/`** — twenty-nine cards on 2026-09-07 (`ls docs/board/ai-review`,
   ...
   whether 3,675 more markers have broken the map's clustering or its label collisions. Nothing reaches `done/` without somebody trying
