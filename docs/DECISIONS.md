...
and explicitly caravan or motorhome capable, which takes 8,501 features down to 3,673 (3,681 until 2026-09-08, when card 0020 dropped the six `access=members` sites it had been labelling instead of excluding, and 3,675 until later the same day, when it dropped the two sites named as scout sites that carry no `scout` tag): an untagged
...
**Also decided here:** the campsite tab never shows an open or closed badge. 98 of 3,673 records
