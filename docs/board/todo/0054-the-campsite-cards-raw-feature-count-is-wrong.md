...
## Tasks
- [ ] Count the `elements` arrays in the three files under `data/raw/osm/` and confirm the total
...
- [ ] Correct the figure in criterion #2 of `docs/board/in-progress/0020-campsites-tab-from-openstreetmap.md`
      (or whichever lane holds it by then), changing the count and nothing else about the sentence.
- [ ] Add the assertion to `scripts/selftest.js`, next to the `card 0036` prose-count block: read the
