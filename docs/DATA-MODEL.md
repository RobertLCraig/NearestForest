...
```json
{
  ...
  "counts": { "campsite": 3574 },
  "counts_by_country": { "England": 2524, "Scotland": 496, "Wales": 554 },
  "licence": "ODbL 1.0",
...
- **`opening_summary` is always absent, and the app must never show an open/closed badge here.**
  Measured 2026-09-08: 97 of 3,574 records publish any opening text at all. A badge would be a guess,
...
- **An absent key means "not known".** `sites.json` writes an explicit `null`; this file omits the
  key, because at 3,574 records the nulls were the majority of the bytes. Every consumer tests
...
From 8,501 OSM elements across the three countries, 3,574 records survive. Every exclusion is
...
| private, members-only or no public access | 103 | `access=members` joined `private` and `no` on 2026-09-08. A club site you are not a member of is not somewhere you can pull up for the night, and the parser had been labelling those six "Members only" rather than dropping them. **72 more were found later the same day by their name alone**, the tag being as sparse here as `scout` was: a Certificated Location (CL) or Certificated Site (CS) is licence-exempt only because an exempted organisation runs it for its members, so members-only is the scheme's definition rather than a policy, and none of the 71 carried the tag. The line that must not move is the full network Club Sites, which take non-members at a higher price, so `club` is deliberately not in the pattern. **One more was found later the same day for the `private` word itself**, which had only ever been read off the `access` tag: "King Edward Park(private)" carries no access tag at all. That pattern reads the **name only**, because `operator=Private` in OSM means privately owned rather than closed — "Llyn Gwynant Campsite" carries it and takes anyone who books. |
