...
- **`opening_summary` is always absent, and the app must never show an open/closed badge here.**
  Measured 2026-09-08: 97 of 3,575 records publish any opening text at all. A badge would be a guess,
...
- **An absent key means "not known".** `sites.json` writes an explicit `null`; this file omits the
  key, because at 3,575 records the nulls were the majority of the bytes. Every consumer tests
...
From 8,501 OSM elements across the three countries, 3,575 records survive. Every exclusion is
...
| private, members-only or no public access | 102 | `access=members` joined `private` and `no` on 2026-09-08. A club site you are not a member of is not somewhere you can pull up for the night, and the parser had been labelling those six "Members only" rather than dropping them. **72 more were found later the same day by their name alone**, the tag being as sparse here as `scout` was: a Certificated Location (CL) or Certificated Site (CS) is licence-exempt only because an exempted organisation runs it for its members, so members-only is the scheme's definition rather than a policy, and none of the 71 carried the tag. The line that must not move is the full network Club Sites, which take non-members at a higher price, so `club` is deliberately not in the pattern. |
